...
pub fn plot_histogram_without_unit(window:&Window,total_count:i64,data_map:&mut HashMap<String,Vec<PlotData>>,stmt:&mut Statement,_graph_condition:&GraphCondition)->Result<(),Box<dyn Error>>{
    //dataキーに値を入れる
    data_map.entry("data".to_string()).or_insert(vec![]);
...
pub fn plot_histogram_with_unit(window:&Window,total_count:i64,data_map:&mut HashMap<String,Vec<PlotData>>,stmt:&mut Statement,_graph_condition:&GraphCondition)->Result<(),Box<dyn Error>>{
...
pub fn plot_densityplot_with_unit(window:&Window,_total_count:i64,data_map:&mut HashMap<String,Vec<PlotData>>,stmt:&mut Statement,graph_condition:&GraphCondition)->Result<HashMap<String,(f64,f64)>,Box<dyn Error>>{
    // ユニットごとのグリッド情報を格納
    ...
    for (index,(unit_name,data_points)) in unit_data_map.iter().enumerate(){
        ...
        if (index+1) % 10 == 0 {
            report_progress(
                ...
                "processing",
                40,
                &format!("{}/{} ユニット処理完了", index+1,unit_data_map.len())
            );
        }
    }
